...
namespace KreditModul
{
    public class modulKredit
    {
        ...
        {
            long amount = Convert.ToInt64(crdAmount * 100);
            int runtime = Convert.ToInt32(crdRuntime);
            long rate = Convert.ToInt64(amount / runtime);
            ...
            output[5] = Convert.ToDouble(interest * runtime) / 100;
        ...
        public List<string> rateAmount(double crdAmount, double crdZins, double crdRate)
        {
            ...
            int runtime = Convert.ToInt32(amount / rate) + 1;
            List<double> output = new List<double> { amount / 100, crdZins, crdRate, amount / rate, 0, 0 };
            long interest;
            double faktor = 0;
            for (int i = 1; i <= runtime; i++)
            {
